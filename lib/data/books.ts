...
export const getBooks = async <K extends (keyof Book)[]>(
  fields: K,
  options: Partial<ContentQueryParams> = {}
): Promise<Pick<Book, ArrayElement<K>>[]> => {
...
export const getBookBySlug = async <K extends (keyof Book)[]>(
  slug: string[],
  fields: K
): Promise<Pick<Book, ArrayElement<K>>> => {
  const baseBooks = await getBookFiles();
